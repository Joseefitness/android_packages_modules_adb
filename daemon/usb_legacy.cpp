...
// Forward-declare the usb_handle helpers used by UsbBlockingConnection below
...
// Daemon-side blocking adapter around the usb_handle function pointers, for
// wrapping in BlockingConnectionAdapter and attaching to an atransport.
namespace {
struct UsbBlockingConnection : public BlockingConnection {
    explicit UsbBlockingConnection(usb_handle* h) : handle_(h) {}

    ~UsbBlockingConnection() override { usb_close(handle_); }

    bool Read(apacket* packet) override {
        if (usb_read(handle_, &packet->msg, sizeof(amessage)) != sizeof(amessage)) {
            PLOG(ERROR) << "remote usb: read terminated (message)";
            return false;
        }
        if (packet->msg.data_length) {
            if (packet->msg.data_length > MAX_PAYLOAD) {
                LOG(ERROR) << "remote usb: read overflow (data length = "
                           << packet->msg.data_length << ")";
                return false;
            }
            packet->payload.resize(packet->msg.data_length);
            if (usb_read(handle_, &packet->payload[0],
    ...
    bool Write(apacket* packet) override {
        int size = packet->msg.data_length;
        if (usb_write(handle_, &packet->msg, sizeof(packet->msg)) !=
            sizeof(packet->msg)) {
            PLOG(ERROR) << "remote usb: 1 - write terminated";
            return false;
        }
        if (size != 0 && usb_write(handle_, packet->payload.data(), size) != size) {
    ...
    bool DoTlsHandshake(RSA*, std::string*) override {
        LOG(FATAL) << "Not supported yet.";
        return false;
    }

    void Close() override { usb_kick(handle_); }
    void Reset() override {
        usb_reset(handle_);
        usb_kick(handle_);
    }
...
static void usb_legacy_ffs_open_thread(usb_handle* usb) {
    ...
    while (true) {
        ...
        LOG(INFO) << "registering usb transport (legacy ffs)";
